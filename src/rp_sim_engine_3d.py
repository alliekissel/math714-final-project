...
class rpSimEngine3D:
    ...
    def coordinate_partitioning(self):
        self.initialize_plotting()
        nb = self.nb
        nc = self.nc
        h = self.h
        ...
        psi = np.block([[self.get_M(), np.zeros((3*nb, 4*nb)), np.zeros((3*nb, nb)), Phi_r.T],
                        [np.zeros((4*nb, 3*nb)), self.get_J_P(), P.T, Phi_p.T],
                        [np.zeros((nb, 3*nb)), P, np.zeros((nb, nb)), np.zeros((nb, nc))],
                        [Phi_r, Phi_p, np.zeros((nc, nb)), np.zeros((nc, nc))]])

        # get indices of dependent generalized coordinates (pivot columns)
        ind_indices = Matrix(self.get_phi_q()).rref()[1]

        # dependent generalized coordinates: u^u (equal to number of constraints)

        # independent generalized coordinates: u^v (equal to nDOFs)

        # velocity kinematic constraint equation: Ku= \nu 
        # with K = [Phi_r Pi] and u = V\hat{u}^v + U\hat{u}^u
        # Then, K = [Phi_r^v Pi^v]\hat{u}^v + [Phi_r^u Pi^u]\hat{u}^u

        #####------- algorithm -------#####
        # solve to find initial accelerations and lagrange multipliers, vector z
        z = np.linalg.solve(psi, eom_rhs)

        # integrate independent coordinates with explicit Forward Euler to get velocity and position
        
        # compute position-level dependent coordinates through iterative Newton step

        # compute velocity-level dependent coordinates through kinematic constrain equation

        # compute again the accelerations (solve linear system EOM)... and repeat
